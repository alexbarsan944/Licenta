...
upper_body = cv2.CascadeClassifier('data/haarcascade_upperbody.xml')
...
while 1:
    ...
    if len(faces) == 0 and len(faces_bw) == 0:
        ...
        print(0)
        p = os.path.sep.join([name + '/', "{}.png".format(str(total).zfill(5))])
        cv2.imwrite(p, roi_gray)
        total += 1
    else:
        ...
        if len(mouth_rects) == 0:
            ...
            for (mx, my, mw, mh) in mouth_rects:
                ...
                if y < my < y + h:
                    # Face and Lips are detected but lips coordinates are within face cordinates which `means lips
                    # prediction is true and person is not waring mask
